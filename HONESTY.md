...
| Nansen second-opinion check (phase 3) | **Not working, and this row previously said it was done. It never was.** Code exists in the indexer that would screen flagged wallets against wallet labels, but it calls `api.nansen.ai/v2/wallet/overview`, an endpoint that does not exist, and no key was ever configured. The `WalletLabel` table is empty across the whole chain, and the live site reports this check as NOT ACTIVE. Corrected 2026-09-23. Second correction the same day, because the earlier wording implied Nansen has nothing for Monad: it does. `POST api.nansen.ai/api/v1/profiler/address/first-funder` returns HTTP 402 with an x402 offer of 0.01 USDC per call, and Monad (chain 143) is one of the accepted payment networks. The service is reachable, permissionless and cheap. We have bought zero calls, so we claim nothing. |
